...
for (const packet of document.packets) {
  const reviewAt = new Date(packet.nextReviewAt || packet.freshness?.nextReviewAt);
...
document.lastAutomatedCheckAt = now.toISOString();
await writeFile(file, `${JSON.stringify(document, null, 2)}\n`);
console.log(JSON.stringify({ packets: document.packets.length, expired }));
