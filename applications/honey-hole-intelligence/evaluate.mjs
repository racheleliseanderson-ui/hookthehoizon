...
import { assessOfficialFacts, summarizeConfidence } from '../_shared/freshness.mjs';
...
export function evaluateHoneyHole(input, now = new Date()) {
  ...
  const locationCheck = rejectSensitiveLocation({
  ...
  if (!locationCheck.ok) {
    return {
      ...
      message: 'Exact coordinates, private-access instructions, vulnerable-water identifiers, contributor-sensitive information, and other inference-enabling location data are not accepted. Use a broad public region and current official sources.'
    };
  }
  ...
  const publicResult = {
    ...
    decisionState,
    resultBlocked: decisionState === 'do_not_proceed' || decisionState === 'verify_before_trip',
    confidence,
    hardStops,
    criticalUnknowns,
    unknownFields,
...
function needsCheck(key, evidence) {
  const item = evidence.find((entry) => entry.key === key);
  return Boolean(item && item.confidenceImpact !== 'supported');
}

function humanize(value) {
  return String(value || '').replaceAll('_', ' ').replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase();
}
