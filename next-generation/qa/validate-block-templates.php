<?php
/** Validate that block-theme HTML files parse into named Gutenberg blocks. */
declare(strict_types=1);
if (! defined('ABSPATH')) { fwrite(STDERR, "WordPress must be loaded.\n"); exit(1); }
$theme_root = dirname(__DIR__) . '/theme';
$files = [];
foreach (['templates', 'parts'] as $directory) {
    $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($theme_root . '/' . $directory));
...
$output = wp_json_encode(['validated' => count($files), 'status' => 'pass'], JSON_PRETTY_PRINT);
